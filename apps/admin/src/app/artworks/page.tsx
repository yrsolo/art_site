...
export default function ArtworksPage() {
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        if (response.artworks[0]) {
          const artworkResponse = await apiFetch<{ artwork: Artwork }>(`/api/admin/artworks/${response.artworks[0].id}`);
  ...
  async function saveArtwork() {
    ...
    try {
      ...
      await loadList(artwork.id || undefined);
      setEditing(false);
  ...
  async function toggleGallery(summary: ArtworkSummary) {
    const fullArtwork = artwork.id === summary.id ? artwork : (await apiFetch<{ artwork: Artwork }>(`/api/admin/artworks/${summary.id}`)).artwork;
