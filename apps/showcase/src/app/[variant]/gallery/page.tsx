...
export default async function VariantGalleryPage({ params }: { params: Promise<{ variant: string }> }) {
  ...
  if (manifest.id === "cold-mist") return <ColdMistGallery manifest={manifest} content={content} artworks={artworks} />;
  if (manifest.id === "copper-glow") return <CopperGlowGallery manifest={manifest} content={content} artworks={artworks} />;
  if (manifest.id === "mint-rose") return <MintRoseGallery manifest={manifest} content={content} artworks={artworks} />;
