...
export function generateStaticParams() {
  return variantManifests.filter((variant) => variant.supportedRoutes.includes("contacts")).map((variant) => ({ variant: variant.id }));
}
...
export default async function VariantContactsPage({ params }: { params: Promise<{ variant: string }> }) {
  ...
  return <VariantRouteClient variantId={variant} route="contacts" />;
}
