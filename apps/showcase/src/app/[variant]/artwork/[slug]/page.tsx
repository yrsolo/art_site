...
export default async function VariantArtworkPage({ params }: { params: Promise<{ variant: string; slug: string }> }) {
  ...
  if (manifest.id === "cold-mist") return <ColdMistDetail manifest={manifest} content={content} artwork={artwork} />;
  if (manifest.id === "copper-glow") return <CopperGlowDetail manifest={manifest} content={content} artwork={artwork} artworks={listPublicArtworks()} />;
  if (manifest.id === "mint-rose") return <MintRoseDetail manifest={manifest} content={content} artwork={artwork} artworks={listPublicArtworks()} />;
